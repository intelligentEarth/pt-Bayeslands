...
class ptReplica(multiprocessing.Process):
    def __init__(self,  num_param, vec_parameters, minlimits_vec, maxlimits_vec, stepratio_vec,   check_likelihood_sed ,  swap_interval, sim_interval, simtime, samples, real_elev,  real_erodep_pts, erodep_coords, filename, xmlinput,  run_nb, tempr, parameter_queue,event , main_proc,   burn_in): 
        ...
        self.runninghisto = False  # if you want to have histograms of the chains during runtime in pos_variables folder NB: this has issues in Artemis
    ...
    def likelihood_func(self,input_vector ):
        ...
        avg_rmse_el = np.average(rmse_elev)
        avg_rmse_er = np.average(rmse_erodep)

        #print ('avg_tausq_elev', avg_rmse_el)
    ...
    def run(self):
        ...
        v_proposal = self.vec_parameters # initial values of the parameters to be passed Badlands
        v_current = v_proposal # to give initial value of the chain

        #Getting initial predictions from Blackbox model
        ...
        #calculate initial likelihood with initial parameters
        ...
        #additional parameters to determine swaps
        init_count = 0
        pt_samples = samples * 0.5
        ...
        for i in range(samples-1):
            ...
            #Check if enough of chains at each temperature have been used to calc posterior
            if i < pt_samples:
            #    #print(i, pt_samples, self.temperature, "i less than pt_samples")
                self.adapttemp = self.temperature
            if i == pt_samples and init_count == 0:
            #    #print(i,pt_samples,self.temperature, "i==pt_samples")
                self.adapttemp = 1
            ...
            [likelihood_proposal, predicted_elev,  pred_erodep_pts, likl_without_temp, avg_rmse_el, avg_rmse_er] = self.likelihood_func(v_proposal)
            #print("xxx pred elev",predicted_elev)
            #print("xxx pred erod",pred_erodep_pts)
            #Good here
            ...
            likeh_list[i+1,0] = likl_without_temp #changed from likelihood_proposal

            #print((i % self.swap_interval), i,  self.swap_interval, ' mod swap')

            if u < mh_prob: # Accept sample
                #print (i,likelihood_proposal,self.temperature, num_accepted, ' accepted - sample,likelihood,temperature,number accepted')
                ...
                likeh_list[i + 1,1] = likelihood_proposal  # contains  all proposal liklihood (accepted and rejected ones)
                ...
                num_div += 1
                    ...
                    #list_xslicepred[i+1,:]=   final_predtopo[xmid, :]  # slice taken at mid of topography along x axis 
                    #list_erodep[i+1,:] = pred_erodep
                    #print("accept i>burn",final_predtopo[xmid, :])
                    #for x in range(self.sim_interval.size): 
                    #    list_erodep_time[i+1,x, :] = pred_erodep_pts[self.sim_interval[x]]
                
                #else:
                    #print(i," sample burning accepted")
                    #list_yslicepred[i+1,:] =  np.inf 
                    #list_xslicepred[i+1,:]=   np.inf
                    #list_erodep[i+1,:] = np.inf
                    #for x in range(self.sim_interval.size): 
                    #    list_erodep_time[i+1,x, :] = np.inf

                    #rmse_elev[i+1,] = np.inf
                    #rmse_erodep[i+1,] = np.inf


            else: # Reject sample
                #print("Rejected ", i,likelihood_proposal,self.temperature, num_accepted)
                ...
                num_div += 1
                    ...
                    #list_xslicepred[i+1,:]=   list_xslicepred[i,:]
                    #list_erodep[i+1,:] = list_erodep[i,:]
                    #print("reject i>burn",list_xslicepred[i,:])
                    #list_erodep_time[i+1,:, :] = list_erodep_time[i,:, :]

                #else:
                 #   print(i," sample burning rejected")
                    #list_yslicepred[i+1,:] =  np.inf 
                    #list_xslicepred[i+1,:]=   np.inf
                    #list_erodep[i+1,:] = np.inf
                    #list_erodep_time[i+1,:, :] = np.inf
            ...
            if ( i % self.swap_interval == 0 ):
                ...
                temperature = np.asarray([self.adapttemp])
                param = np.concatenate([v_current,others,np.asarray([self.adapttemp])])     
        ...
        #print("xxx list_xslicepred", list_xslicepred)
...
class ParallelTempering:
    def __init__(self, vec_parameters, num_chains, maxtemp,samples,swap_interval, fname, realvalues_vec, num_param,  real_elev, erodep_pts, erodep_coords, simtime, siminterval, resolu_factor, run_nb, inputxml,check_likelihood_sed):
        self.swap_interval = swap_interval
        self.folder = fname
    ...
    # assigin temperatures dynamically   
    ...
    def initialize_chains (self, minlimits_vec, maxlimits_vec, stepratio_vec,  check_likelihood_sed,   burn_in):
        ...
        for i in xrange(0, self.num_chains):
            self.vec_parameters =   np.random.uniform(minlimits_vec, maxlimits_vec) # will begin from diff position in each replica 
            self.chains.append(ptReplica( self.num_param, self.vec_parameters, minlimits_vec, maxlimits_vec, stepratio_vec,  check_likelihood_sed ,self.swap_interval, self.sim_interval,   self.simtime, self.samples, self.real_elev,   self.real_erodep_pts, self.erodep_coords, self.folder, self.xmlinput,  self.run_nb,self.temperatures[i], self.parameter_queue[i],self.event[i], self.wait_chain[i],burn_in))
    ...
    def swap_procedure(self, parameter_queue_1, parameter_queue_2):
        ...
        if parameter_queue_2.empty() is False and parameter_queue_1.empty() is False:
            ...
            #SWAPPING PROBABILITIES ##XXXcheck on Monday 20181023
            #swap_proposal =  (lhood1/[1 if lhood2 == 0 else lhood2])*(1/T1 * 1/T2)
            #print("old swap", swap_proposal)
            try:
    ...
    def run_chains (self ):
        ...
        if self.show_fulluncertainity == True: # this to be used when you need output of the topo predictions - 5th and 95th percentiles
            ...
            print("run optimal,",optimal_para)
        ...
        return (pos_param,likelihood_rep, accept_list,   combined_erodep,  pred_topofinal, rmse_elev, rmse_erodep)


    def view_crosssection_uncertainity(self,  list_xslice, list_yslice):
        print ('list_xslice', list_xslice.shape, list_xslice)
    ...
    def show_results(self, filename):
        ...
        likehood_rep = np.zeros((self.num_chains, self.samples - burnin, 2 )) # index 1 for likelihood posterior and index 0 for Likelihood proposals. Note all likilihood proposals plotted only
        ...
        for i in range(self.num_chains):
                file_name = self.folder + '/posterior/pos_parameters/'+'chain_' + str(self.temperatures[i]) + '.txt'
                ...
                file_name = self.folder + '/posterior/pos_likelihood/'+'chain_' + str(self.temperatures[i]) + '.txt'
                dat = np.loadtxt(file_name) 
                likehood_rep[i, :] = dat[burnin:]

                file_name = self.folder + '/posterior/accept_list/' + 'chain_' + str(self.temperatures[i]) + '.txt'
                dat = np.loadtxt(file_name) 
                accept_list[i, :] = dat 

                file_name = self.folder + '/posterior/accept_list/' + 'chain_' + str(self.temperatures[i]) + '_accept.txt'
                dat = np.loadtxt(file_name) 
...
def plot_erodeposition(erodep_mean, erodep_std, groundtruth_erodep_pts, sim_interval, fname):
    ...
    #print('xxx erodep_mean',erodep_mean)
    ## the bars
...
def main():
    ...
    if problem == 1:
        ...
        #maxlimits_vec = [2.0,real_erod,m,n,real_caerial,real_cmarine]
        #minlimits_vec = [1.0,real_erod,m,n,real_caerial,real_cmarine]

        maxlimits_vec = [3.0,7.e-6, 2, 2,  1.0, 0.7]  
        minlimits_vec = [0.0 ,3.e-6, 0, 0, 0.6, 0.3 ]   

        #if you want to freeze a parameter, keep max and min limits the same
        vec_parameters = np.random.uniform(minlimits_vec, maxlimits_vec) #  draw intial values for each of the free paramete
        ...
        print(true_parameter_vec.shape[0])
        ...
        stepsize_ratio  = 0.2 #   you can have different ratio values for different parameters depending on the problem. Its safe to use one value for now
    ...
    #-------------------------------------------------------------------------------------
    pos_param,likehood_rep, accept_list, combined_erodep, pred_elev, rmse_elev, rmse_erodep = pt.run_chains()

    #print("XXX combined_erodep",combined_erodep)
    ...
    for i in range(sim_interval.size):
            ...
            #print("XXX pos_ed",combined_erodep) 
    ...
    #Determine rmse stats
    print ('rmse shape before  ', rmse_elev.shape)
    x_index = np.where(rmse_elev==np.inf)  
    rmse_elev = np.delete(rmse_elev, x_index, axis = 0) 
    if rmse_elev[:].size > 0:
        rmse_el = np.mean(rmse_elev[:])
        rmse_el_std = np.std(rmse_elev[:])
        rmse_el_min = np.amin(rmse_elev[:])
    else:
        print('rmse_elev is empty')
        rmse_el=0
        rmse_el_std=rmse_el
        rmse_el_min=rmse_el
    print ('rmse shape after inf', rmse_elev.shape)

    x_index = np.where(rmse_erodep==np.inf)  
    rmse_erodep = np.delete(rmse_erodep, x_index, axis = 0) 
    ...
    if rmse_elev[:].size > 0:
        ...
        print('rmse_er is empty')    
        rmse_er=0
        rmse_er_std=rmse_er
        rmse_er_min=rmse_er

    plt.plot(rmse_elev[:, 0],  label='Elev')
    plt.plot(rmse_erodep[:, 0],   label='Erodep') 

    plt.xlabel('Samples', fontsize=18)
    plt.ylabel(' RMSE', fontsize=18)
    plt.legend(loc='upper right')
    plt.savefig(fname+'/rmse_samples.png') 
    plt.clf()

    #Make master result file
    resultingfile = open(problemfolder+'/master_result_file.txt','a+')  
    
    time_total = (timer_end-timer_start)/60
    
    labels = np.asarray(['Problem ', 'Samples', 'Maxtemp', 'Swap Interval', 
        'Surrogate Interval','Surrogate Probability', 'Total time', 'RMSE elev',
        'RMSE erodep', 'RMSE elev std', 'RMSE erdp std', 'RMSE elev min',
        'RMSE erdp min', 'RMSE surrogate'])
    allres =  np.asarray([ problem, samples, maxtemp, swap_interval,
                          0, 0, time_total, rmse_el, 
                          rmse_er, rmse_el_std, rmse_er_std, rmse_el_min, 
                          rmse_er_min, 0 ]) 

    #allres =  np.asarray([ problem, samples, maxtemp, swap_interval, 0, 0, time_total, rmse_el, rmse_er, rmse_el_std, rmse_er_std, rmse_el_min, rmse_er_min, 0 ]) 
