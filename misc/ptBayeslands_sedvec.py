...
class ptReplica(multiprocessing.Process):
	...
	def run(self):
		...
		with file(('%s/description.txt' % (self.filename)),'a') as outfile:
			...
			outfile.write('\n\tstep_vec: {0}'.format(self.stepratio_vec))   
...
,  math.exp(eta_erodep_pts_pro))
			...
			if u < mh_prob: # Accept sample
				print (v_proposal,   i,likelihood_proposal, step_eta_elev , math.exp(eta_elev_pro), self.temperature, num_accepted, ' is accepted - rain, erod, step rain, step erod, likh')
